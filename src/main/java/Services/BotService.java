...
public class BotService {
        ...
        if (bot.getSize() < 25) {
            if (inWorld()) {
                ...
                var teleList = gameState.getGameObjects().stream()
                        .filter(item -> item.getGameObjectType() == ObjectTypes.TELEPORTER
                                && Math.abs(getHeadingBetweenObject(item, bot)) <= 135)
                        .sorted(Comparator.comparing(item -> getDistanceBetween(bot, item)))
                        .collect(Collectors.toList());
                ...
                if (!teleList.isEmpty() && !playerListBySize.isEmpty() && getDistanceBetween(bot,
                    ...
                    if (differenceHeading > 0) {
                        ...
                        && getDistanceBetween(bot, torpedoList.get(0)) <= 1.2 * dangerFromEpidermis) {
                    differenceHeading = torpedoList.get(0).getCurrentHeading()
                            - getHeadingBetweenObject(torpedoList.get(0), bot);
                    if (Math.tan(Math.toRadians(Math.abs(differenceHeading))) <= Math
                            .abs(bot.getSize() / getDistanceBetween(bot, torpedoList.get(0)))
                            && getDistanceBetween(bot, torpedoList.get(0)) <= 120 && bot.getShieldCount() > 0
                            && bot.getSize() > 50) {
                        ...
                        && getDistanceBetween(bot, gasList.get(0)) - (bot.getSize() + gasList.get(0).getSize()) < 10) {
            ...
            if (inWorld()) {
                ...
                if (!teleList.isEmpty() && !playerListBySize.isEmpty() && getDistanceBetween(bot,
                    ...
                    if (differenceHeading > 0) {
                        playerAction.action = PlayerActions.FORWARD;
                        playerAction.heading = (bot.getCurrentHeading() + 100 - Math.abs(differenceHeading) + 360)
                                % 360;
                    } else {
                        playerAction.action = PlayerActions.FORWARD;
                        playerAction.heading = (bot.getCurrentHeading() - 100 + Math.abs(differenceHeading) + 360)
    ...
    private boolean inWorld() {
        if (gameState.world.radius != null) {
            if (Math.sqrt(bot.getPosition().x * bot.getPosition().x + bot.getPosition().y * bot.getPosition().y
                    + 5 * bot.getSize() * bot.getSize()) + 100 < gameState.getWorld().getRadius()) {
                return true;
            } else {
                return false;
            }
    ...
    private PlayerAction greedByFood(PlayerAction playerAction) {
        var playerelse = gameState.getPlayerGameObjects().stream()
                .filter(item -> item.size != bot.size)
                .sorted(Comparator.comparing(item -> getDistanceBetween(item, bot)))
                .collect(Collectors.toList());
        ...
        // comparator=Comparator.thenComparing(Comparator.comparing(item ->
        // item.getPosition().y));
        var food = gameState.getGameObjects()
                .stream()
                .filter(item -> (item.getGameObjectType() == ObjectTypes.SUPERFOOD
                        || item.getGameObjectType() == ObjectTypes.FOOD) &&
                        item.getPosition().x * item.getPosition().x + item.getPosition().y * item.getPosition().y
                                + 5 * bot.getSize() * bot.getSize() < gameState.getWorld().getRadius()
    ...
    private PlayerAction greedByOffense(PlayerAction playerAction) {
        ...
        if (bot.getSize() >= 25 && !gameState.getGameObjects().isEmpty()) {
            ...
            // else if (bot.getTorpedoCount() == 0 && bot.getTeleCount() > 0 &&
            // !all.isEmpty()){
